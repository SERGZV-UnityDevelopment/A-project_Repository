// Этот скрипт вторая версия контроллера вешаеться на CharecterBasis и выполняет движения повороты и анимацию персонажа
using UnityEngine;
...
public class PlayerControllerTwo : MonoBehaviour 
{
	...
	void FixedUpdate()
	{
		if(Inv.InventoryOn == false) // Если инвентарь включен
		{
			AnimationState(); 			// Определяем какое состояние анимации будет проигрываться
			RestrictionsAnimations(); 	// Ограничиваем проигрыш определённых анимаций в определённых условиях
			PlayerRotation();			// Вращение персонажа
			...
			SmoothRunWalk();	// В случае необходимости делаем плавный переход от шага к бегу и от бега к шагу
		}
		else if(Inv.InventoryOn == true) // Иначе Если инвентарь включен
		{
			SpeedRun = 0;
...
//-----------------------------------------------------------------------------------------------------------------------------------------------
	...
	void AnimationState()	// Этот метод занимаеться переключением состояний анимаций - шаг, бег, красться, ползти.
	{
...
//-----------------------------------------------------------------------------------------------------------------------------------------------
	...
	void RestrictionsAnimations()	// Этот метод ограничиваем проигрыш определённых анимаций в определённых условиях
	{
	...
	//	__Walk_FB = DownHorizontal == false ? __Walk_FB = Input.GetAxis("Vertical") : __Walk_FB = 0;
		__Walk_FB = Input.GetAxis("Vertical");
...
//-----------------------------------------------------------------------------------------------------------------------------------------------
	...
	void PlayerRotation()	// Физичесское вращение персонажа
	{
...
//-----------------------------------------------------------------------------------------------------------------------------------------------
	...
	void SmoothRunWalk()	// Плавный переход от шага к бегу и от бега к шагу
	{
		...
		//	SmoothDampingAnimation(SpeedRun);
